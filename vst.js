...
var BASE_URL, YATTA_LOADED, bring_to_back, bring_to_top, check_time_conflict, click_event, delete_sect, generate_table, get_random_color, get_rowspan, go_to, hover_in_event, hover_out_event, process_datetime, tc, tt, tt_preview, visible,
  __slice = [].slice;
...
if (YATTA_LOADED) {
  ...
  go_to = function(dept, ccode, call_back) {
    ...
    $('div#classes').load(url + ' div#classes', function() {
      ...
      $('tr.sectodd, tr.secteven').css({
        'cursor': 'pointer'
      }).unbind('click').click(click_event).hover(hover_in_event, hover_out_event);
      if (!ccode) {
        $(window).scrollTop(0);
      } else {
  ...
  get_rowspan = function(elem) {
    var $first_row, $rest, $row, $rows, course_code, course_color, course_name, datetime, room, sect, section, td_arr, timeslots, _i, _len;
    timeslots = [];
  ...
  click_event = function() {
    var l, lesson, no_time_conflict, timeslots, _i, _j, _k, _l, _len, _len1, _len2, _len3, _ref;
    if (!visible) {
    ...
    if (!no_time_conflict) {
      for (_k = 0, _len2 = timeslots.length; _k < _len2; _k++) {
        lesson = timeslots[_k];
  ...
  generate_table = function() {
    var DAYS, SAT, SUN, col, dow, earliest_start_time, empty_cell, header, l, latest_end_time, make_div_content, make_time_str, range, row, _i, _j, _k, _l, _len, _len1, _len2, _len3, _m, _n, _o, _p, _ref, _ref1, _ref2, _ref3;
    earliest_start_time = Math.min.apply(Math, [18].concat(__slice.call((function() {
      ...
      var _i, _len, _ref, _ref1, _results;
      _ref1 = (_ref = []).concat.apply(_ref, tt_preview);
      _results = [];
      for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
        l = _ref1[_i];
        _results.push(l.start_time);
    ...
    SUN = tt[6].length !== 0 || tc[6].length !== 0 || tt_preview[6].length !== 0;
    SAT = tt[5].length !== 0 || tc[5].length !== 0 || tt_preview[5].length !== 0;
    if (SUN) {
      DAYS.push('SAT', 'SUN');
    } else if (SAT) {
      DAYS.push('SAT');
    ...
    for (dow = _k = 0; 0 <= range ? _k < range : _k > range; dow = 0 <= range ? ++_k : --_k) {
      ...
      for (_l = 0, _len1 = _ref1.length; _l < _len1; _l++) {
        l = _ref1[_l];
        $("<div class='time-conflict' style='background-color: rgba(255, 0, 0, 0.3); height: " + ((l.end_time - l.start_time + 1) * 20 + l.end_time - l.start_time) + "px'>\u00A0</div>").appendTo("\#r" + l.start_time + " \#c" + (l.dow + 1) + " div.outer");
      }
    }
    for (dow = _m = 0; 0 <= range ? _m < range : _m > range; dow = 0 <= range ? ++_m : --_m) {
      _ref2 = tt_preview[dow];
      for (_n = 0, _len2 = _ref2.length; _n < _len2; _n++) {
        l = _ref2[_n];
        $("<div id='" + l.section + "' class='mydiv' style='height: " + ((l.end_time - l.start_time + 1) * 20 + l.end_time - l.start_time) + "px'>" + (make_div_content(l)) + "</div>").appendTo("\#r" + l.start_time + " \#c" + (l.dow + 1) + " div.outer");
      }
    }
    for (dow = _o = 0; 0 <= range ? _o < range : _o > range; dow = 0 <= range ? ++_o : --_o) {
      _ref3 = tt[dow];
  ...
  */
  ...
  $('tr.sectodd, tr.secteven').css({
    'cursor': 'pointer'
  }).click(click_event).hover(hover_in_event, hover_out_event);
