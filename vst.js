...
if (!VST_LOADED) {
  ...
  generate_table = function() {
    var DAYS, SAT, SUN, col, dow, earliest_start_time, empty_cell, header, l, latest_end_time, make_div_content, make_time_str, range, row, tableDOM, _i, _j, _k, _l, _len, _len1, _ref, _ref1;
    ...
    tableDOM = DOMinate([
      document.getElementById('container'), [
        'table#vst', ['thead'].concat((function() {
          ...
          for (row = _i = earliest_start_time; earliest_start_time <= latest_end_time ? _i <= latest_end_time : _i >= latest_end_time; row = earliest_start_time <= latest_end_time ? ++_i : --_i) {
            _results.push(["tr\#r" + row].concat([['td', make_time_str(row)]], (function() {
              var _j, _results1;
              _results1 = [];
              for (col = _j = 1; 1 <= range ? _j <= range : _j >= range; col = 1 <= range ? ++_j : --_j) {
                _results1.push(["td\#c" + col, empty_cell]);
              }
              return _results1;
            })()));
          }
    ...
    for (dow = _i = 0; 0 <= range ? _i < range : _i > range; dow = 0 <= range ? ++_i : --_i) {
      ...
      for (_j = 0, _len = _ref.length; _j < _len; _j++) {
        ...
        })(l.section)).appendTo("\#r" + l.start_time + " \#c" + (l.dow + 1));
      }
    }
    for (dow = _k = 0; 0 <= range ? _k < range : _k > range; dow = 0 <= range ? ++_k : --_k) {
      ...
      for (_l = 0, _len1 = _ref1.length; _l < _len1; _l++) {
        l = _ref1[_l];
        $("<div class='time-conflict' style='height: " + ((l.end_time - l.start_time + 1) * 20 + l.end_time - l.start_time) + "px'>\u00A0</div>").appendTo("\#r" + l.start_time + " \#c" + (l.dow + 1)).click();
      }
    }
    return false;
  };
